...
def trainer(train, valid, test, n_chars=33, img_w=128, max_len=27, feature_maps=100,  
    ...
    zero_vec = np.zeros(img_w).astype(theano.config.float32)
...
if __name__=="__main__":  
    ...
    if data == "predefined_split":
        ...
        x = cPickle.load(open('./data/data_using_predefined_split.py','rb'))

        #with open('./data/data_using_predefined_split.py','rb') as f:
            #x = cPickle.loads(f)
        #x = cPickle.load("./data/data_using_predefined_split.py","rb")
